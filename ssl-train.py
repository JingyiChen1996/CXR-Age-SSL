...
# import dataset
from mixmatch_dataset import train_val_split, train_val_split_labeled, NIH_CXR_BASE, CxrDataset, CXR_unlabeled
...
best_test_prec1 = 0
best_test_prec1_t = 0
acc1_tr, acc5_tr, losses_tr = [], [], []
losses_cl_tr, losses_x_tr, losses_u_tr = [], [], []
acc1_val, acc5_val, losses_val, losses_et_val = [], [], [], []
acc1_test, acc5_test, losses_test, losses_et_test = [], [], [], []
acc1_t_tr, acc1_t_val, acc1_t_test = [], [], []
learning_rate, weights_cl, weights = [], [], []

def main():

    global args, best_prec1, best_test_prec1, best_test_prec1_t
    global acc1_tr, acc5_tr, losses_tr 
    global losses_cl_tr, losses_x_tr, losses_u_tr
    global acc1_val, acc5_val, losses_val, losses_et_val
    global acc1_test, acc5_test, losses_test, losses_et_test
    ...
    if args.model == 'baseline':
        num_classes = train_val_split_labeled('data/NIH_train_10000.csv')
        train_labeled_set = CxrDataset(NIH_CXR_BASE, "data/train_8000.csv")
        val_set = CxrDataset(NIH_CXR_BASE, "data/val_2000.csv")
    else: 
        num_classes = train_val_split('data/NIH_train_10000.csv', n_labeled = args.n_labeled, split=args.split)
        train_labeled_set = CXR_unlabeled(NIH_CXR_BASE, "data/train_labeled_1000.csv")
        train_unlabeled_set = CXR_unlabeled(NIH_CXR_BASE, "data/train_unlabeled_7537.csv")
        val_set = CxrDataset(NIH_CXR_BASE, "data/validation_1576.csv")

    test_set = CxrDataset(NIH_CXR_BASE, "data/NIH_test_2500.csv") 
    ...
    if args.resume:
        ...
        logger.set_names(['Train Loss','Valid Loss','Train Acc.1','Train Acc.5', \
                        'Valid Acc.1','Valid Acc.5','Test Loss','Test Acc.1','Test Acc.5'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.model == 'baseline':
            ...
            prec1_tr, prec5_tr, loss_tr = train_sup(labeled_trainloader, model, criterions, optimizer, epoch, args)
            weight_cl = 0.0
        elif args.model == 'pi':
            print('Pi model')
            prec1_tr, prec5_tr, loss_tr, loss_cl_tr, weight_cl = train_pi(labeled_trainloader, unlabeled_trainloader, num_classes, model, criterions, optimizer, epoch, args)
        elif args.model == 'mt':
            print('Mean Teacher model')
            prec1_tr, prec5_tr, loss_tr, loss_cl_tr, prec1_t_tr, weight_cl = train_mt(labeled_trainloader, unlabeled_trainloader, num_classes, model, model_teacher, criterions, optimizer, epoch, args)
        elif args.model == 'mixmatch':
            print('MixMatch model')
            prec1_tr, prec5_tr, loss_tr, loss_x_tr, loss_u_tr, weight = train_mixmatch(labeled_trainloader, unlabeled_trainloader, num_classes, model, optimizer, ema_optimizer, epoch, args)

        # evaluate on validation set 
        if args.model == 'mixmatch':
            prec1_val, prec5_val, loss_val = validate(val_loader, model, criterions, args, 'valid')
            prec1_ema_val, prec5_ema_val, loss_ema_val = validate(val_loader, ema_model, criterions, args, 'valid')
            prec1_test, prec5_test, loss_test = validate(test_loader, model, criterions, args, 'test')
            prec1_ema_test, prec5_ema_test, loss_ema_test = validate(test_loader, ema_model, criterions, args, 'test')
        else: 
            prec1_val, prec5_val, loss_val = validate(val_loader, model, criterions, args, 'valid')
            prec1_test, prec5_test, loss_test = validate(test_loader, model, criterions, args, 'test')
        ...
        acc5_tr.append(prec5_tr)
        losses_tr.append(loss_tr)
        acc1_val.append(prec1_val)
        acc5_val.append(prec5_val)
        losses_val.append(loss_val)
        acc1_test.append(prec1_test)
        acc1_test.append(prec5_test)
        ...
        writer.add_scalar('loss/test_loss', loss_test, (epoch+1)) 

        writer.add_scalar('accuracy/train_acc', prec1_tr, (epoch+1))
        writer.add_scalar('accuracy/val_acc', prec1_val, (epoch+1))
        writer.add_scalar('accuracy/test_acc', prec1_test, (epoch+1))  
        if args.model=='mt':
            writer.add_scalar('accuracy/test_t_acc', prec1_t_test, (epoch+1))
        if args.model=='mixmatch':
            writer.add_scalar('accuracy/val_ema_acc', prec1_ema_val, (epoch+1))
            writer.add_scalar('accuracy/test_ema_acc', prec1_ema_test, (epoch+1))

        # append logger file
        logger.append([loss_tr, loss_val, prec1_tr, prec5_tr, \
                       prec1_val, prec5_val, loss_test, prec1_test, prec5_test])
...
def train_pi(label_loader, unlabel_loader, num_classes, model, criterions, optimizer, epoch, args, weight_pi=20.0):
    ...
    return top1.avg , top5.avg, losses.avg, losses_pi.avg, weights_cl.avg
...
def train_mt(label_loader, unlabel_loader, num_classes, model, model_teacher, criterions, optimizer, epoch, args, ema_const=0.95, weight_mt=8.0):
    ...
    return top1.avg , top5.avg, losses.avg, losses_cl.avg, top1_t.avg, weights_cl.avg
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   ...
                   ws=weights, top1=top1, top5=top5))
